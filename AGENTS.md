...
```
cross-cutting/controls.yaml        # GC-0A, GC-0B, GC-0C — audit, traceability, agent registry
cross-cutting/feedback-loops.yaml  # Re-entry paths for incidents and changes
```

### Stage-Specific Files

| Stage | Directory | Controls |
|-------|-----------|---------|
| 1 — Intent Ingestion | `stages/01-intent-ingestion/` | QC-1A, QC-1B, RC-1A, SC-1A, AC-1A, GC-1A |
| 2 — System Design | `stages/02-system-design/` | QC-2A, RC-2A, SC-2A, SC-2B, AC-2A |
| 3 — Coding & Implementation | `stages/03-coding-implementation/` | QC-3A, QC-3B, RC-3A, SC-3A, SC-3B, SC-3C, GC-3A |
| 4 — Testing & Documentation | `stages/04-testing-documentation/` | QC-4A, QC-4B, QC-4C, RC-4A, SC-4A, SC-4B, AC-4A |
| 5 — Deployment & Release | `stages/05-deployment-release/` | QC-5A, RC-5A, RC-5B, SC-5A, SC-5B |
| 6 — Observability & Maintenance | `stages/06-observability-maintenance/` | QC-6A, RC-6A, SC-6A, SC-6B, AC-6A |

Each stage directory contains:
- `README.md` — human-readable description, key tasks, and controls table
- `controls.yaml` — full machine-readable control definitions

### Full Control Registry
```
controls/registry.yaml    # Flat index of all 30 controls — use for fast lookup by ID
```
